...
{
  echo '<a href="insta_login3.php?page='.$page.'&btn='.$_GET['btn'].'">&nbsp;&nbsp;&nbsp;'.$page.' </a>';
}
}
else
{
...
//echo $check['id'];
if($checkqqq){
  $query2qw = mysqli_query($conn,"SELECT id FROM gallary where name = '$search' order by id desc");
...
//echo $check['id'];
if($checkqqq){
    $query2qw = mysqli_query($conn,"SELECT id FROM gallary where tags like '%$search%' order by id desc");
$pp = mysqli_num_rows($query2qw);
$res_per_page=2;
$no_of_pages=ceil($pp/$res_per_page);
